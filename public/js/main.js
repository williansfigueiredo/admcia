...
window.calcularAlertaVencimento = function(dataVencimento, status) {
  ...
  try {
    ...
    const criarIconeAlerta = (cor, titulo) => {
      return `<svg title="${titulo}" style="width: 18px; height: 16px; margin-left: 4px; vertical-align: middle; cursor: default;" viewBox="0 0 16 14">
        <path fill="${cor}" d="M7.938 2.016A.13.13 0 0 1 8.002 2a.13.13 0 0 1 .063.016.146.146 0 0 1 .054.057l6.857 11.667c.036.06.035.124.002.183a.163.163 0 0 1-.054.06.116.116 0 0 1-.066.017H1.146a.115.115 0 0 1-.066-.017.163.163 0 0 1-.054-.06.176.176 0 0 1 .002-.183L7.884 2.073a.147.147 0 0 1 .054-.057z"/>
        <path fill="#000" d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
      </svg>`;
    };
    ...
    if (diffDays < 0) {
      ...
      return criarIconeAlerta('#dc3545', 'VENCIDO!');
    } else if (diffDays === 0) {
      // Vence HOJE - VERMELHO
      console.log('⚠️ VENCE HOJE! Retornando alerta');
      return criarIconeAlerta('#dc3545', 'VENCE HOJE!');
    } else if (diffDays === 1) {
      // Vence amanhã - AMARELO
      console.log('⚠️ VENCE AMANHÃ! Retornando alerta');
      return criarIconeAlerta('#ffc107', 'VENCE AMANHÃ!');
    } else if (diffDays === 2) {
